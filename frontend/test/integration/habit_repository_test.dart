  ...
  group('HabitRepository', () {
    ...
    group('createHabit', () {
      test('calls POST /habits with correct body', () async {
        ...
        when(
          () => mockDio.post(ApiEndpoints.habits, data: request.toJson()),
        ).thenAnswer(
          (_) async => Response(
            requestOptions: RequestOptions(path: ApiEndpoints.habits),
            statusCode: 201,
    ...
    group('updateHabit', () {
      test('calls PUT /habits/:id with correct body', () async {
        ...
        when(
          () =>
              mockDio.put(ApiEndpoints.habitById('h1'), data: request.toJson()),
        ).thenAnswer(
          (_) async => Response(
            requestOptions: RequestOptions(path: ApiEndpoints.habitById('h1')),
            statusCode: 200,
            data: {
              'id': 'h1',
              'title': 'Updated Habit',
    ...
    group('deleteHabit', () {
      test('calls DELETE /habits/:id', () async {
        when(() => mockDio.delete(ApiEndpoints.habitById('h1'))).thenAnswer(
          (_) async => Response(
            requestOptions: RequestOptions(path: ApiEndpoints.habitById('h1')),
            statusCode: 204,
          ),
    ...
    group('completeHabit', () {
      test('calls POST /completions with habit_id', () async {
        when(
          () =>
              mockDio.post(ApiEndpoints.completions, data: {'habit_id': 'h1'}),
        ).thenAnswer(
          (_) async => Response(
      ...
      test('includes note when provided', () async {
        ...
        final completion = await repository.completeHabit(
          'h1',
          note: 'Great session',
        );
    ...
    group('getStreak', () {
      test('calls GET /completions/streak/:id', () async {
        when(() => mockDio.get(ApiEndpoints.streak('h1'))).thenAnswer(
          (_) async => Response(
            requestOptions: RequestOptions(path: ApiEndpoints.streak('h1')),
            statusCode: 200,
            data: {
              'habit_id': 'h1',
              'current_streak': 5,
    ...
    group('getCompletions', () {
      ...
      test('includes habit_id filter when provided', () async {
        when(
          () => mockDio.get(
            ApiEndpoints.completions,
            queryParameters: {'page': 1, 'limit': 100, 'habit_id': 'h1'},
          ),
        ).thenAnswer(
          (_) async => Response(
            requestOptions: RequestOptions(path: ApiEndpoints.completions),
    ...
    group('uncompleteHabit', () {
      test('calls DELETE /completions/:id', () async {
        when(
          () => mockDio.delete(ApiEndpoints.completionById('c-1')),
        ).thenAnswer(
          (_) async => Response(
